...
interface ProjectItem {
    text: string;
    subList?: string; // Optional sublist item
}
...
const Career: React.FC = () => {
	...
	const projectItems: ProjectItem[] = [
        { text: 'Product attribute and settings automated testing', subList: 'Eradicated critical data input errors' },
        { text: 'Inventory management reporting and automation', subList: 'Decreased purchasing labour by ~80%' },
        { text: 'Sales management plugin with AJAX shopping cart integration', subList: 'Increased AOV by 8.3%' },
        { text: 'Bespoke ID verification software and WooCommerce integration', subList: 'Decreased Credit Card fraud by 98%' }
    ];

    const projectItems2: ProjectItem[] = [
        { text: 'Full functionality interactive shopping cart to replace cart page', subList: 'Increased conversions by 0.7%' },
        { text: 'Complex multi-state animated menus represented in an elegant UI', subList: 'Strong brand confidence booster with state of the art menu' },
        { text: 'Design and development of the site and merchandising strategy optimized for market', subList: '7.1% overall conversion rate' },
    ];
					...
					<article className={career.companyPositions}>
						...
						<div className={career.position}>
							<div className={career.positionContent}>
								...
								{projectItems.map(({ text, subList }, index) => (
									<li key={index}>
										{text}
										{subList && (
											<span className={career.subList}>
											<span className={career.bullet}></span>{subList}
											</span>
										)}
										</li>
									))}
									</ul>
								<Badges list={fullStack} color="" block="stack" fullContainer />
							</div>
							<div className={career.positionAlt}></div>
						</div>
