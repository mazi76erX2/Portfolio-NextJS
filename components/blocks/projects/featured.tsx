...
interface ProjectContent {
    ...
    description: string;
    stack: string[];
    imageOptions: any;  // Adjust as needed based on your imageOptions structure
    images: {
        key: string;
        url: string;
        hover: 'left' | 'right';
		...
		<m.section
			...
			<div className={css.details}>
				<div className={css.projectHeader}>
					...
					<m.div variants={viewProjectVariant} className={css.viewProject}>
			...
			<div className={css.imageContainer}>
				<span className={`${css.imageAnimationContainer}`}>
					{images.map(({ key, url, hover, h, w }, index) => {
						...
						return (
							<m.div key={`${index}-${key}`} variants={item} whileHover="hover">
								<Image src={url} alt={`${project} screenshot ${index}`} height={h} width={w} />
							</m.div>
						);
					})}
				</span>
