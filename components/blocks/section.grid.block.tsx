...
function generateGrid(gridSize: number, classProp: string): JSX.Element[] {
	const grid: JSX.Element[] = [];
	for (let i = 0; i < gridSize; i++) { // Corrected loop condition to exclude the extra element
	  grid.push(
		<div key={`grid-line-${i}`} className={classProp} />
	  );
	}
	return grid;
  }
